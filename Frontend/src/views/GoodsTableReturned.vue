...
<template>
    ...
    <div class="modal-dialog">
      <div class="modal-content">
        <div class="modal-header">
          <h1 class="modal-title fs-5">Add Received Bag</h1>
          <button
            type="button"
            class="btn-close"
          ...
          <form>
            <div class="form-group">
              ...
              <input
                type="text"
                class="form-control"
                v-model="bagNumberSearch"
                placeholder="Bag Number"
              />
              <label for="fault" class="mt-2">Fault</label>
              <span>
                <select class="form-select" v-model="selectedFault">
                  <option default :value="null"></option>
                  <option value="Employee Fault">Employee Fault</option>
                  <option value="TSS Fault">TSS Fault</option>
                </select>
              </span>
            </div>
          </form>
        ...
        <div class="modal-footer d-flex justify-content-center">
          <button
            ...
            data-bs-dismiss="modal"
            class="btn btn-primary"
            @click="filterTable(selectedFault)"
            v-if="selectedFault !== null && bagNumberSearch.trim() !== ''"
          >
            Add
          </button>
        </div>
  ...
  <!-- <div class="form-group">
    <label for="bagNumberSearch">Search by Bag Number:</label>
    <input
      type="text"
      class="form-control"
      v-model="bagNumberSearch"
    ...
    <template #table-row="props">
      ...
      >
        <button
          ...
          data-bs-toggle="modal"
          :data-bs-target="'#' + props.row.bagNumber"
        >
          Received
        </button>
        <div
          class="modal fade mt-5"
          :id="props.row.bagNumber"
          tabindex="-1"
          aria-hidden="true"
        >
          <div class="modal-dialog">
...
export default {
  ...
  methods: {
    addSearchedItem() {
      if (this.searchedItem && !this.addedItems.includes(this.searchedItem)) {
        this.rows.push(this.searchedItem);
        this.addedItems.push(this.searchedItem);

        this.bagNumberSearch = "";
        this.searchedItem = null;
      }
    },
    filterTable(selectedFault) {
      console.log(this.bagNumberSearch);
      if (this.bagNumberSearch === "") {
        this.searchedItem = null;
      } else {
        ...
        apiInstance
          .put(
            `https://localhost:7164/BagInventory/ReturnBagFault/${this.bagNumberSearch}`,
            {
              fault: selectedFault,
            }
          )
          .then((result) => {
            if (result.status >= 200 && result.status <= 205) {
              this.searchedItem = result.data;
              this.refreshData();
              $toast.success(
                `Bag added. Fault assigned to ${this.bagNumberSearch}`
              );
            }
          })
    ...
    ReturnGood(id, quantityPassed, quantityFailed, notes) {
      apiInstance
        ...
        .catch(
          (err) =>
            $toast.error(
              "Quantity issued not equal to QuantityFailed + QuantityPassed"
            ) + err
        );
    },
    ...
    refreshData() {
      fetch("https://localhost:7164/BagInventory/GetReturnedBags")
        .then((res) => {
          if (res.ok) {
            return res.json();
